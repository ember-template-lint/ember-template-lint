...
const { ruleURL } = require('./../rules/base');
...
module.exports = function generateRuleTests({
  ...
  groupingMethod(name, function () {
    ...
    function parseMeta(item) {
      ...
      meta.moduleId = 'moduleId' in meta ? meta.moduleId : ConfigDefaults.moduleId;
    ...
    function parseResult(result) {
      ...
      if (typeof result.moduleId !== 'string') {
        defaults.moduleId = ConfigDefaults.moduleId;
        delete result.moduleId;
      }

      if (typeof result.url !== 'string') {
        defaults.url = ruleURL(name);
        delete result.url;
      }
