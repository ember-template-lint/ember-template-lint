...
const { ERROR_SEVERITY, IGNORE_SEVERITY } = require('../get-config');
...
module.exports = function generateRuleTests({
  ...
  groupingMethod(name, function() {
    ...
    function updateLinterConfig(localConfig) {
      ...
      const severity = localConfig === false ? IGNORE_SEVERITY : ERROR_SEVERITY;

      linter.config.rules[name] = { config: localConfig, severity };
    }
