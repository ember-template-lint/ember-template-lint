...
class Linter {
  ...
  buildASTPlugins(config) {
    ...
    let configuredRules = this.config.rules || rules;
    for (let pluginName in configuredRules) {
      if (configuredRules[pluginName] === false) {
        continue;
      }
      if (!(pluginName in rules)) {
        throw new Error(
          `ember-template-lint: Config rules error - unable to find linter for rule, named: "${pluginName}".`
        );
      }
