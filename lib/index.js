...
const { searchAndExtractHbs } = require('extract-tagged-template-literals');
...
class Linter {
  ...
  verify(options) {
    ...
    try {
      const source = stripBom(options.source);
      const preprocessStr =
        options.moduleId.endsWith('.js') || options.moduleId.endsWith('.ts')
          ? searchAndExtractHbs(source)
          : source;
