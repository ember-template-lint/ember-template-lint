...
class Linter {
  ...
  buildVisitors(config) {
    let { moduleName, rawSource, results } = config;
    ...
    let visitors = [TransformDotComponentInvocation];
    ...
    for (let pluginName in rules) {
      ...
      visitors.push(() => {
        plugin.templateEnvironmentData = { moduleName, rawSource };
        return plugin.getVisitor();
      });
    }
  ...
  verify(options) {
    ...
    try {
      ...
      // run the lint rules as ast visitors
      let visitors = this.buildVisitors(pluginsConfig);
      for (let visitor of visitors) {
        let { ast: lintedAst } = transform(ast, visitor);
        ast = lintedAst;
      }
    } catch (error) {
      let message = {
