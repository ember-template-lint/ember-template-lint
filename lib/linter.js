...
class Linter {
  ...
  _buildRule(ruleName, options, addToResults) {
    ...
    let ruleOptions = Object.assign(
      {
        name: ruleName,
        config: options.configRules[ruleName],
  ...
  findRulesForPath(overrides, filePath) {
    if (overrides && filePath) {
      return overrides.find(
        override => override.files && micromatch.isMatch(filePath, override.files)
      );
    }
  }
  ...
  buildRules(options) {
    ...
    // Overrides related.
    const overridesObj = this.findRulesForPath(this.config.overrides, options.filePath) || {};
    const { rules: overrideRuleset } = overridesObj;
    ...
    options.configRules = configuredRules;
