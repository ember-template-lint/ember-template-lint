...
const { searchAndExtractHbs } = require('@lifeart/ember-extract-inline-templates');
const { parse: rawParseJS } = require('@babel/parser');
...
class Linter {
  ...
  async verify(options) {
    ...
    if ((options.filePath || '').endsWith('.js') || (options.filePath || '').endsWith('.ts')) {
      try {
        source = searchAndExtractHbs(source, {
          parse(code) {
            return rawParseJS(code, {
              sourceType: 'module',
              plugins: [],
            });
          },
