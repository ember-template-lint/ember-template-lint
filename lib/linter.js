...
import { extractTemplates } from './extract-templates.js';
import PrettyPrinter from './formatters/pretty.js';
...
// We need to disable some rules when parsing embedded templates
// This is because some of the errors are expected to be caught by ESLint
const MUTED_RULES_FOR_EMBEDDED_TEMPLATES = new Set([
  // ESLint will determine whether or not a template is using a local variable which will not require the keyword this
  'no-implicit-this'
]);
...
export default class Linter {
  ...
  async verify(options) {
    ...
    let rawSource = hasBOM ? options.source.slice(1) : options.source;

    let templateInfos = extractTemplates(rawSource, options.filePath);

    for (let templateInfo of templateInfos) {
      let source = templateInfo.template;
      let templateAST;
      ...
      for (let rule of rules) {
        try {
          if (
            templateInfo.isEmbeddedTemplate &&
            MUTED_RULES_FOR_EMBEDDED_TEMPLATES.has(rule.ruleName)
          ) {
            continue;
          }
          let visitor = await rule.getVisitor();
          ...
          for (let result of rule.results) {
            result.line += templateInfo.line;
            result.endLine += templateInfo.line;
            // result.column += templateInfo.column;
            //   result.endColumn += templateInfo.endColumn;
          }
