...
export default class Linter {
  ...
  async _applyEmbeddedFix(ruleNames, options, fileConfig) {
    let templateInfos = extractTemplates(options.source, options.filePath);
    ...
    //
    // This is basically an Array.prototype.map on the templateInfos, but async
    let transformed = [];
    for (let templateInfo of templateInfos) {
      let currentSource = templateInfo.template;
      for (let ruleName of ruleNames) {
        let rule = this._buildRule(ruleName, {
          shouldFix: true,
          filePath: options.filePath,
      ...
      // all rules have ran on the extracted <template> contents
      transformed.push({ templateInfo, transformed: currentSource });
    }

    let newSource = replaceTemplates(options.source, transformed);
  ...
  async verify(options) {
    ...
    let results = [];
    let templateInfos = [];
    let filePath = options.filePath ?? './assumed.hbs';
    let fileConfig = this._moduleStatusCache.getConfigForFile(options);
    ...
    try {
      templateInfos = extractTemplates(source, filePath);
    } catch (error) {
      let message = buildErrorMessage(options.filePath, error);
