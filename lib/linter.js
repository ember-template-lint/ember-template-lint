...
export default class Linter {
   ...
   */
  async loadConfig() {
    const config = await getProjectConfig(this.workingDir, this.options);
    this.setupConfig(config);
  }

  get _todoHandler() {
    return this.__todoHandler || (this.__todoHandler = new TodoHandler(this.workingDir));
  }
   ...
   */
  buildRules(templateInfo, externalOptions, fileConfig) {
    const options = {
      filePath: externalOptions.filePath,
      configResolver: externalOptions.configResolver,
  ...
  #byType = {
    // js + ts
    script: {
      check: (source, options, fileConfig) => {
        ...
        for (let templateInfo of templateInfos) {
          let templateResults = this._verifyTemplateSync(templateInfo, options, fileConfig);
      ...
      fix: (ruleNames, options, fileConfig) => {
        return syncMapOverTemplatesInScript(options.source, (source, coordinates) => {
          ...
          for (let ruleName of ruleNames) {
            ...
            let visitor = rule.getSyncVisitor();
    ...
    template: {
      check: (source, options, fileConfig) => {
        ...
        return this._verifyTemplateSync(templateInfo, options, fileConfig);
      },
      fix: (ruleNames, options, fileConfig) => {
    ...
    glimmerScript: {
      check: (source, options, fileConfig) => {
        ...
        transformer.each((template, coordinates) => {
          let templateResults = this._verifyTemplateSync(
            {
  ...
  async verifyAndFix(options) {
    ...
    return this.verifyAndFixSync(options);
  }

  verifyAndFixSync(options) {
    ...
    do {
      ...
      messages = this.verifySync(options);
      shouldVerify = false;
  ...
  _applyFixes(options, results) {
    ...
    return processFile(filePath, {
      ...
      glimmerScript: () => {
        return this.#byType.glimmerScript.fix(ruleNames, options, fileConfig);
      },
      script: () => {
        ...
        return this.#byType.script.fix(ruleNames, options, fileConfig);
      },
      template: () => {
  ...
  configureTodos() {
    if (!this._todoHandler) {
      throw new Error('Linter config is not loaded, please, call getConfig or loadConfig first');
    }
    return this._todoHandler.checkStorageFile();
  }
  ...
  async verify(options) {
    ...
    return this.verifySync(options);
  }
  ...
  _parseTemplate(template, filePath) {
    ...
    try {
      templateAST = parse(template);
    } catch (error) {
      let message = buildErrorMessage(filePath, error);
      results.push(message);
    }
    return {
      templateAST,
  ...
  _applyOffsetsToEmbeddedTemplates(templateInfo, rule) {
   ...
   * @returns
   */
  _verifyTemplateSync(templateInfo, options, fileConfig) {
    const { results, templateAST } = this._parseTemplate(templateInfo.template, options.filePath);
    if (results.length > 0) {
      return results;
    }

    let { failures, rules } = this.buildRules(templateInfo, options, fileConfig);
    ...
    for (let rule of rules) {
      try {
        ...
        this._processStrictModeResults(templateInfo, rule);
        this._applyOffsetsToEmbeddedTemplates(templateInfo, rule);
  ...
  _processStrictModeResults(templateInfo, rule) {
