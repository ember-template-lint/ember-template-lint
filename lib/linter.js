...
class Linter {
  ...
  async verifyAndFix(options) {
    ...
    let doctypeMatch = originalSource.match(DOCTYPE_REGEX);
    let hasDoctype = doctypeMatch !== null;

    let currentSource = hasBOM ? originalSource.slice(1) : originalSource;
    currentSource = hasDoctype ? currentSource.replace(doctypeMatch, '') : currentSource;
    ...
    let output = hasDoctype ? doctypeMatch[0] + currentSource : currentSource;
    output = hasBOM ? byteOrderMarkPrefix + output : output;
  ...
  async verify(options) {
    ...
    source = source.replace(DOCTYPE_REGEX, '');
