...
import ReportUnusedDisableDirectives from './rules/_report-unused-disable-directives.js';
...
export default class Linter {
  ...
  _buildRule(ruleName, options) {
    ...
    let configuredRules = options.configuration ?? fileConfig.rules[ruleName];
    let Rule = options.Rule ?? fileConfig.loadedRules[ruleName];
  ...
  buildRules(options) {
    ...
    let reportUnusedDisableDirectives =
      typeof options.reportUnusedDisableDirectives === 'boolean'
        ...
        : fileConfig?.reportUnusedDisableDirectives ?? false;

    // Our normal behavior is to report violations as part of the rules when they are run.
    // However, if the rule is not enabled globally, the rule will never run. Here we add
    // a special rule handler that will report unused disable directives for these globally
    // disabled rules as well.
    if (reportUnusedDisableDirectives) {
      let activeRules = Object.entries(fileConfig.rules)
        .filter(([, config]) => config.severity > 0)
        .map(([name]) => name);
      let rule = this._buildRule('report-unused-disable-directives', {
        ...options,
        activeRules,
  ...
  #byType = {
    ...
    glimmerScript: {
      ...
      fix: async (ruleNames, options, fileConfig) => {
        let t = new Transformer(options.source);
