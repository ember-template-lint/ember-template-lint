...
class Linter {
  ...
  _determineSeverityForRule(ruleName, pendingStatus, configRule) {
    const severity = configRule && configRule.severity ? configRule.severity : ERROR_SEVERITY;
    if (typeof pendingStatus === 'boolean') {
      return pendingStatus ? WARNING_SEVERITY : severity;
    ...
    let ruleOptions = Object.assign(
      {
        ...
        defaultSeverity: this._determineSeverityForRule(ruleName, options.pending, fileConfig),
