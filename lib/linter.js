...
export default class Linter {
  ...
  async verify(options) {
    ...
    let allDisabledRules = new Set();
    let allUsedDisabledRules = new Set();
    ...
    for (let templateInfo of templateInfos) {
      ...
      for (let rule of rules) {
        try {
          ...
          // Collect disabled rules info
          if (rule._disabledRules) {
            for (let node of rule._disabledRules) {
              allDisabledRules.add(node);
            }
            for (let node of rule._usedDisabledRules) {
              allUsedDisabledRules.add(node);
            }
          }
    ...
    if (this.config.rules['no-unused-disable']?.severity !== IGNORE_SEVERITY) {
      for (let node of allDisabledRules) {
        if (!allUsedDisabledRules.has(node)) {
          results.push({
            rule: 'no-unused-disable',
            severity: ERROR_SEVERITY,
