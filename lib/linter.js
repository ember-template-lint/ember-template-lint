...
import { templateInfoForTemplateFile, templateInfoForScript } from './template-info.js';
...
export default class Linter {
  ...
  async _applyFixes(options, results) {
    ...
    let isTemplateTag = isGlimmerFileExtension(options.filePath);

    if (isTemplateTag) {
      return await this._applyEmbeddedFix(ruleNames, options, fileConfig);
    }

    return await this._applyStandaloneFix(ruleNames, options, fileConfig);
  }

  async _applyStandaloneFix(ruleNames, options, fileConfig) {
    let currentSource = options.source;
  ...
  async _applyEmbeddedFix(ruleNames, options, fileConfig) {
    let t = new contentTagUtils.Transformer(options.source);

    // for each embedded template, run the full set of rules over that template.
    // after we do this for each embedded template, we'll use the byte-indexes
    // from content-tag, and whole-sale replace the entirety of each template
    // to reduce the amount of index/slice math we have to do for each file.
    await t.transformAll(async (source, coordinates) => {
  ...
  async verify(options) {
    ...
    if (isGlimmerFileExtension(filePath)) {
      return this.verifyGJS(source, options, fileConfig);
    }

    if (isHBS(filePath) || isHTML(filePath)) {
      return this.verifyTemplate(source, options, fileConfig);
    }
    ...
    if (options.checkHbsTemplateLiterals && isScript(filePath)) {
      return this.verifyHbsTemplateLiterals(source, options, fileConfig);
    }
  ...
  async verifyGJS(source, options, fileConfig) {
    ...
    await transformer.transformAll(async (template, coordinates) => {
      ...
      try {
        ...
        if (templateInfo.isStrictMode) {
          for (let result of rule.results) {
            const resultCoordinates = contentTagUtils.reverseInnerCoordinates(templateInfo, result);
