...
const errorMessage = (role, validParents) => {
  return `You have an element with the role of "${role}" but it is missing the required (immediate) parent element of "[${validParents.join(
    ', '
...
const CHILDS_FOR_ANALYZE = [].concat.apply([], Object.keys(ROOTS).map(keyName => ROOTS[keyName]));
...
module.exports = class RequiredContextRole extends Rule {
  visitor() {
    return {
      ElementNode(node) {
        ...
        if (CHILD_PROP_KEY in node) {
          ...
          if (!opts.logged) {
            opts.logged = true;
            this.log({
              message: errorMessage(opts.value, opts.validParentNames),
        ...
        const childs = node.children
          .map(child => {
            ...
            if (CHILDS_FOR_ANALYZE.indexOf(childRoleValue) === -1) {
              return false;
            }
            return {
        ...
        childs.forEach(child => {
          if (VALID_PARENTS_FOR_CHILD[child.value].indexOf(roleValue) === -1) {
            child.node[CHILD_PROP_KEY] = {
              validParentNames: VALID_PARENTS_FOR_CHILD[child.value],
              value: child.value,
              logged: false,
            };
          }
