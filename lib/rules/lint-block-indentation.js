...
module.exports = function(addonContext) {
  ...
  return class BlockIndentation extends buildPlugin(addonContext, 'block-indentation') {
    parseConfig(config) {
      let configType = typeof config;

      let errorMessage = 'The block-indentation rule accepts one of the following values.\n ' +
    ...
    validateBlockEnd(node) {
      ...
      let correctedEndColumn = endColumn - displayName.length - controlCharCount + startOffset;
      ...
      if(correctedEndColumn !== startColumn) {
        ...
        let warning = `Incorrect indentation for \`${displayName}\` beginning at ${startLocation}` +
    ...
    validateBlockElse(node) {
      ...
      let startColumn = node.loc.start.column;
      let elseStartColumn = inverse.loc.start.column;

      if(elseStartColumn !== startColumn) {
        let displayName = node.path.original;
        let startLocation = `L${node.loc.start.line}:C${node.loc.start.column}`;
        let elseLocation = `L${inverse.loc.start.line}:C${inverse.loc.start.column}`;

        let warning = `Incorrect indentation for inverse block of \`{{#${displayName}}}\` beginning at ${startLocation}` +
