...
function getAllElementNodeDescendants(node) {
  return AstNodeInfo.childrenFor(node).flatMap((childNode) => {
    const isNotPresentationRole =
      ...
      'presentation';
    const isNotNoneRole =
      AstNodeInfo.attributeTextValue(AstNodeInfo.findAttribute(childNode, 'role')) !== 'none';
    const isNotHidden =
      AstNodeInfo.attributeTextValue(AstNodeInfo.findAttribute(childNode, 'aria-hidden')) !==
      'true';
    ...
    if (isNotPresentationRole && isNotNoneRole && isNotHidden && childNode.type === 'ElementNode') {
