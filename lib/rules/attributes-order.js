...
export default class AttributesOrder extends Rule {
  ...
  getAppliedOrder(nodeTokens) {
    ...
    const attributeOrderMinusSplattributes = [
      ...this.config.attributeOrder.filter(
        (attribute) => attribute !== AttributeType.SPLATTRIBUTES
      ),
    ];

    const splattributeLastIndex = nodeTokens[AttributeType.SPLATTRIBUTES]?.last_index;
    const attributeLastIndex = nodeTokens[AttributeType.ATTRIBUTES]?.last_index;

    if (
      splattributeLastIndex === maxLastIndex ||
      (splattributeLastIndex && splattributeLastIndex > attributeLastIndex)
    ) {
      this.appliedOrder = [...attributeOrderMinusSplattributes, AttributeType.SPLATTRIBUTES];
    } else if (
      splattributeLastIndex === minLastIndex ||
      (splattributeLastIndex && splattributeLastIndex < attributeLastIndex)
    ) {
      this.appliedOrder = [AttributeType.SPLATTRIBUTES, ...attributeOrderMinusSplattributes];
    } else {
      this.appliedOrder = [...attributeOrderMinusSplattributes, AttributeType.SPLATTRIBUTES];
    }
    return this.appliedOrder;
  ...
  createUnorderedErrorMessage(nodeTokens, attributeType, source) {
    ...
    if (attributeOrder === 1 || attributeOrder === 2) {
      ...
      } and before ${this.getAppliedOrder(nodeTokens)[attributeOrder + 1]}
            `;
    }
    return `${this.capitalizedAttribute(attributeType)} ${source} must go after ${
