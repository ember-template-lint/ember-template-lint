...
module.exports = class BaseRule {
  ...
  set templateEnvironmentData(env) {
    ...
    this.source = env.rawSource.split(reLineEnds);
  ...
  _isRuleName(name) {
    ...
    return rules[name] || name === this.ruleName;
  }
  ...
  getVisitors() {
    ...
    let visitors = {};
    let ruleVisitors = this.visitors();
    ...
    astHandlers.CommentStatement.enter.after.push(function(node) {
      ...
      if (loggedNodes.indexOf(node) === -1) {
        let locationInfo = calculateLocationDisplay(pluginContext.templateEnvironmentData.moduleName, {
    ...
    astHandlers.MustacheCommentStatement.enter.after.push(function(commentNode) {
      ...
      let parentNode = _.last(elementStack);
  ...
  _processInstructionNode(node) {
    ...
    if (m) {
      ...
      if (instructionArgs) {
        ...
        if (names.indexOf(this.ruleName) === -1) {
  ...
  _processLegacyConfigNode(node) {
    ...
    for (let key in hash) {
      ...
      if (key === this.ruleName && value === 'false') {
        for (i = 0; i < this._configStack.length; i++) {
          this._configStack[i] = false;
        }
        this.config = false;
  ...
  log(result) {
    ...
    let reportedResult = assign({}, defaults, result);
