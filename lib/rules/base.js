...
module.exports = function(options) {
  let allRules = require('./index');
  let _console = options.console || console;
  let log = options.log;
  let config = options.config;
  let ruleName = options.name;
  let defaultSeverity = options.defaultSeverity;
  ...
  BasePlugin.prototype.getVisitors = function() {
    ...
    astHandlers.CommentStatement.enter.after.push(function(node) {
      ...
      if (loggedNodes.indexOf(node) === -1) {
        let locationInfo = calculateLocationDisplay(pluginContext.options.moduleName, {
  ...
  BasePlugin.prototype._processInstructionNode = function(node) {
    ...
    if (m) {
      ...
      if (instructionArgs) {
        ...
        if (loggedNodes.indexOf(node) === -1) {
          let badNames = names.filter(function(name) {
  ...
  BasePlugin.prototype._processLegacyConfigNode = function(node) {
    ...
    let hash = hashParts.reduce(function(memo, part) {
