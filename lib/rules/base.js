...
module.exports = class BaseRule {
  ...
  getVisitor() {
    ...
    // Manage scope
    astHandlers.BlockStatement.enter.before.push(node => {
      scope.pushFrame(node);
    });
    astHandlers.BlockStatement.exit.after.push(node => {
    ...
    astHandlers.ElementNode.keys.children.enter.before.push(function(elementNode) {
      scope.pushFrame(elementNode);
    });
    astHandlers.ElementNode.exit.after.push(function(elementNode) {
      if (elementNode !== scope.currentNode) {
        throw new Error('Element stack out of sync with AST!');
      }
      scope.popFrame();
    });
