...
module.exports = class BaseRule {
  constructor(options) {
    ...
    let configData = this.processConfigForSeverity(options.config);
  ...
  _determineConfigForSeverity(config) {
    switch (config) {
      case 'off':
        return false;
      case 'warn':
  ...
  /**
   * Sets the severity of the rule based on user configuration
   * and returns the config based on the severity OR as defined by the user.
   * Eg:
   * {'no-implicit-this': 'warn'}
   * This would set the severity to 1 and return `true`.
   * { 'no-implicit-this': { "severity": "warn", "config": "allow": [ "fooData" ] } }
   * This would set the severity of the rule to 1 and return the user defined config back.
   * { 'no-implicit-this': { "severity": "warn"} }
   * This would set the severity of the rule to 1 and return true.
   * @param {*} configData
   */
  processConfigForSeverity(configData) {
    let severityConfig;
    // In case of {'no-implicit-this': 'off|warn|error'}
    if (typeof configData === 'string') {
      severityConfig = this._determineConfigForSeverity(configData);
      return severityConfig !== undefined ? severityConfig : configData;
    }

    // In case of object based config with or without severity information.
    if (typeof configData === 'object') {
      // Support for config along with severity:
      if (configData.severity) {
        severityConfig = this._determineConfigForSeverity(configData.severity);
        if (severityConfig === undefined) {
          throw new Error('Severity can only be "off", "warn", or "error"');
        }
        // If user defined config found then return that else return severity based value;
        return configData.config ? configData.config : severityConfig;
      }
    }
    return configData;
  }
  ...
  log(result) {
    ...
    if (this._filePath) {
      defaults.filePath = this._filePath;
    }
    if (this._moduleName) {
