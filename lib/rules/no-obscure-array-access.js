...
function getHelperParams(node) {
  ...
  const firstDigitIndex = node.parts.findIndex((part) => DIGIT_REGEXP.test(part));
        ...
        if (node.original && PATH_REGEXP.test(node.original)) {
          if (this.mode === 'fix') {
            ...
            if (path.parentNode.type === 'MustacheStatement') {
              ...
              path.parentNode.params = getHelperParams(
                node,
                node.parts.findIndex((part) => DIGIT_REGEXP.test(part))
              );
            } else {
