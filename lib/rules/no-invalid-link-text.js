...
function hasInvalidLinkText(node) {
  ...
  if (nodeChildren.length !== textChildren.length || isHidden(node) || hasValidAriaLabel(node)) {
  ...
  const hasGenericLinkTexts =
    !nodeChildren.length || linkTexts.some((linkText) => DISALLOWED_LINK_TEXT.has(linkText));
