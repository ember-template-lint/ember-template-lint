...
function ERROR_MESSAGE(tagName, argumentName) {
  ...
  let closestKey = fuzzyResults.length ? `@${fuzzyResults[0].item}` : argumentName;

  if (closestKey in deprecatedArgs) {
    return deprecateArgument(tagName, closestKey, deprecatedArgs[closestKey]);
  }

  if (closestKey in deprecatedEvents) {
    return deprecateEvent(tagName, closestKey, deprecatedEvents[closestKey]);
  }
