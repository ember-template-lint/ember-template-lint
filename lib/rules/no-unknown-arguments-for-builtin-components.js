...
function fix(node, argument, componentName) {
  const tagMeta = KnownArguments[componentName];
  ...
  visitor() {
    const importedComponents =
      this.isStrictMode && this.fullSource
        ? parseImports(this.fullSource, new Set(Object.keys(KnownArguments)), this.filePath)
        : new Map();
    ...
    return {
      ElementNode(node) {
        ...
        } else if (!KnownArguments[tag]) {
          return;
        }

        let nodeMeta = KnownArguments[componentName];
        let warns = [];
        let seen = [];

        const logError = (attr) => {
          this.log({
            message: ERROR_MESSAGE(componentName, attr.name),
            node: attr,
            source: (this.sourceForNode(attr) || '').split('=')[0],
            isFixable: fix(node, attr, componentName),
          });
        };
        ...
        const logRequired = (variants) => {
          this.log({
            message: REQUIRED_MESSAGE(componentName, variants),
            node,
        ...
        for (let argument of attributes) {
          ...
          for (let conflictList of nodeMeta.conflicts) {
            if (conflictList.every((item) => seen.includes(item))) {
              for (let argumentName of conflictList) {
                const attr = attributes.find(({ name }) => `@${argumentName}` === name);
                if (attr) {
