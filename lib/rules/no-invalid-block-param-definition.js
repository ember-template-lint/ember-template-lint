...
module.exports = class NoInputBlock extends Rule {
  _checkForAsPrefix(node) {
    ...
    const maybeInvalidNode = node.blockParams.length === 0;
    if (!maybeInvalidNode) {
      return;
    }
    ...
    const openBlockCloseIndex = pureNodeSource.indexOf('>');
    ...
    if (!isValidEmptyBlock) {
      const txt = pureNodeSource.slice(0, openBlockCloseIndex);
      const msgs = [];
      if (asKeywordIndex === -1) {
