...
module.exports = class AttributeSpacing extends Rule {
  ...
  validateParamsAndHashPairs (node) {
    ...
    */
    ...
    this.iterateParams(node.hash.pairs, 'attribute', expectedLineStart + node.params.length, expectedColumnStart, node);
  }

  getExpectedEndTagLocOfNonBlockForm(node) {
    let endAfterLastAttribute;
    let endOnNextLine;
    let hashPairLength = node.hash.pairs.length;
    let paramsLength = node.params.length;
    let nodeStartColumn = node.loc.start.column;

    if (hashPairLength === 0 && paramsLength === 0) {
      endAfterLastAttribute = {
        line: node.loc.start.line,
        column: `${node.path.original}`.length
      };
      return [endAfterLastAttribute];
    } else if (hashPairLength) {
      endAfterLastAttribute = node.hash.loc.end;
    } else if (paramsLength) {
      endAfterLastAttribute = node.params[paramsLength - 1].loc.end;
    }

    if (this.allowBlocklessMustacheEndOnLastAttribute) {
      return [endAfterLastAttribute];
    } else if (this.allowBlocklessMustacheEndOnLastAttribute === false) {
      endOnNextLine = {
        line: endAfterLastAttribute.line + 1,
        column: nodeStartColumn
      };
      return [endOnNextLine];
    } else {
      return [endAfterLastAttribute, endOnNextLine];
    }
  }
  ...
  validateCloseBrace(node) {
    ...
    */
    ...
    let expectedStartLocations = this.getExpectedEndTagLocOfNonBlockForm(node);

    let componentName = node.path.original;
    let locations = [];

    expectedStartLocations.forEach((location) => {
      if (actualStartLocation.line !== location.line ||
        actualStartLocation.column !== location.column) {
        locations.push(location);
      }
    });

    if (locations.length) {
      let expectedIndentations, positions;

      expectedIndentations = locations.map((location) => {
        return `L${location.line}:C${location.column}`;
      });
      ...
      let message = `Incorrect indentation of close curly braces '}}' for the component '${componentName}' beginning at L${actualStartLocation.line}:C${actualStartLocation.column}. Expected to be at ${positions.join(' or ')} with an indentation of ${expectedIndentations.join(' or ')} but was found at ${actualStartLocation.column}`;
  ...
  validateNonBlockForm(node) {
    ...
    let hasParams = node.params.length || node.hash.pairs.length;
    if (hasParams) {
      this.validateParamsAndHashPairs(node);
    }
    this.validateCloseBrace(node, hasParams);
  }
