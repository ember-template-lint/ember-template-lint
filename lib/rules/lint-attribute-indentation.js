...
module.exports = class AttributeSpacing extends Rule {
  ...
  validateParamsAndHashPairs (node) {
    ...
    */
    ...
    if (typeof this.config === 'object' && this.config["indentation"]) {
        indentation = this.config["indentation"];
    }

    let expectedColumnStart = node.loc.start.column + indentation; //params should be after proper positions from component start node
