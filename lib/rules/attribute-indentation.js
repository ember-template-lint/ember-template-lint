...
export default class AttributeIndentation extends Rule {
  ...
  getGlobalNodeFromNode(node) {
    return node._globalNode || get(parse(this.sourceEdited || this.source.join('')), this.globalPath);
  }
  ...
  fixLine(lines, actual, expected, path) {
    ...
    if (actual.line > expected.line) {
      ...
      const lineAfter = line.substring(actual.column).trimStart();
    ...
    const globalNode = get(parse(this.sourceEdited), this.globalPath);
    const nodeToReturn = get(globalNode, path);
  ...
  getBlockParamStartLoc(node) {
        ...
        */
        ...
        if (hashPairLineEndSource) {
          ...
          */
          actual = {
            line: programStartLoc.line,
            column: getWhiteSpaceLength(this.lines[programStartLoc.line - 1]),
          };
        }
      } else {
      }
    ...
    const endColumn = line.trimEnd().replace(/\s*(}{2}|>)\s*$/, '').length;
    ...
    */
    ...
    return {
      nextLocation: {
        ...
        column: expected.column + location.endColumn - actual.column,
      },
      fixedNode,
    };
  }
  ...
  iterateParams(params, type, expectedLineStart, expectedColumnStart, node, path) {
    ...
    for (let index = 0; index < paramsToFix.length; index += 1) {
      ...
      if (
        ...
        if (this.mode === 'fix') {
          fixedParam = this.fixLine(this.lines, actualStartLocation, { line: expectedLineStart, column: expectedColumnStart }, paramPath);
          fixedNode = get(this.getGlobalNodeFromNode(fixedParam), path);
          paramsToFix = get(this.getGlobalNodeFromNode(fixedParam), currentPath);
        } else {
      ...
      if (this.mode === 'fix' && type === 'MustacheStatement') {
        ...
        if (fixedParam._globalNode) {
          fixedNode = get(fixedParam._globalNode, path);
  ...
  iterateChildren(node, parentPath, childrenPath) {
    let fixedNode = node;
    let children = get(node, childrenPath);
    ...
    for (let index = 0; index < children.length; index += 1) {
      ...
      switch (child.type) {
        case 'ElementNode':
          ...
          fixedNode = get(this.getGlobalNodeFromNode(fixedChild), parentPath);
          wasUpdated = true
          break;
      ...
      if (wasUpdated) {
        children = get(fixedNode, childrenPath);
      }
    }
  ...
  validateElementNode(node, path) {
    if (this.seen.has(node)) return node;

    let fixedNode = node;
    const shouldApplyRule = canApplyRule(node, node.type, this.config) && this.config.processElements;
    if (shouldApplyRule) {
      if (node.modifiers.length > 0 || node.attributes.length > 0) {
        let expectedCloseBraceLocation;
        ({ fixedNode, nextLocation: expectedCloseBraceLocation } = this.validateParams(fixedNode, path));
    ...
    if (shouldApplyRule) {
      if (fixedNode.children.length) {
        ...
        const expectedStartLine =
          lastChild.type.endsWith('Statement')
    ...
    return {
      ...
      BlockStatement(node) {
        try {
          ...
          if (this.mode !== 'fix' || this.template.body.includes(node)) {
            return this.removeGlobalNodeAttribute(this.validateBlockStatement(node));
          }
        } catch (error) {
          console.error(error);
        }
      ...
      MustacheStatement(node) {
        try {
          this.globalPath = ['body', this.template.body.indexOf(node)];
          if (this.mode !== 'fix' || this.template.body.includes(node)) {
            return this.removeGlobalNodeAttribute(this.validateMustacheStatement(node));
          }
        } catch (error) {
          console.error(error);
        }
      ...
      ElementNode(node) {
        try {
          this.globalPath = ['body', this.template.body.indexOf(node)];
          if (this.mode !== 'fix' || this.template.body.includes(node)) {
            const fixedNode = this.removeGlobalNodeAttribute(this.validateElementNode(node));
            return fixedNode
          }
        } catch (error) {
          console.error(error);
