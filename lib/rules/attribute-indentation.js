...
export default class AttributeIndentation extends Rule {
  ...
  fixElementNode(
    ...
    return returnString ? replacementElementSource : fakeElement;
  }
  ...
  fixBlockStatement(
    ...
    console.log('replacementElementSource', replacementElementSource);
  ...
  fixNode(node, options) {
    switch (node.type) {
      ...
      case 'AttrNode':
        ...
        console.log(node.value, newValueWithEqual, newValue, quote);
      ...
      case 'BooleanLiteral':
        return node.value;
      default:
        // console.log('fixNode', node, node.loc);
        return sourceForLoc(node.loc.data && node.loc.data.source.source, node.loc);
    }
  }
  ...
  visitor() {
    ...
    return {
      BlockStatement(node) {
        // console.log('BlockStatement', node, node.hash, node.program);
        if (this.mode === 'fix') {
