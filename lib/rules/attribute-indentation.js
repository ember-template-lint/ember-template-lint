...
export default class AttributeIndentation extends Rule {
  ...
  getGlobalNodeFromNode(node) {
    return (
      node._globalNode || get(parse(this.sourceEdited || this.source.join('')), this.globalPath)
    );
  }
  ...
  validateElementNode(node, path) {
    ...
    const shouldApplyRule =
      canApplyRule(node, node.type, this.config) && this.config.processElements;
    if (shouldApplyRule) {
  ...
  visitor() {
    ...
    return {
      ...
      ElementNode(node) {
        this.globalPath = ['body', this.template.body.indexOf(node)];
        if (this.mode !== 'fix' || this.template.body.includes(node)) {
          return this.removeGlobalNodeAttribute(this.validateElementNode(node));
        }
        return node;
      },
    };
