...
const OPTIONAL_CONFIG_KEYS = new Set(['additionalForbidden', 'message']);
...
function isValidConfigObjectFormat(config) {
  for (let key in config) {
    ...
    if ((key === 'forbidden' || key === 'additionalForbidden') && !valueIsArray) {
      return false;
    } else if (key === 'message' && !typeof value === 'string') {
      return false;
    } else if (!(DEFAULT_CONFIG[key] || OPTIONAL_CONFIG_KEYS.has(key))) {
      return false;
    }
  }
...
function makeForbiddenElements(config) {
  if (Object.prototype.hasOwnProperty.call(config, 'forbidden')) {
    return sanitizeConfigArray(config.forbidden || FORBIDDEN_ELEMENTS);
  } else if (Object.prototype.hasOwnProperty.call(config, 'additionalForbidden')) {
    return sanitizeConfigArray([...config.additionalForbidden, ...FORBIDDEN_ELEMENTS]);
  }
    ...
    switch (configType) {
      ...
      case 'object':
        if (Array.isArray(config)) {
          ...
          return {
            forbidden: makeForbiddenElements(config),
    ...
    return {
      ElementNode(node, path) {
        ...
        if (isForbiddenElement) {
          this.log({
            message: this.config.message || ERROR_MESSAGE_FORBIDDEN_ELEMENTS(node.tag),
