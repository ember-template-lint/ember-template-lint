...
function sanitizeConfigArray(forbidden = []) {
  return forbidden.filter((option) => option !== '').sort((a, b) => b.length - a.length);
}
...
module.exports = class NoForbiddenElements extends Rule {
    ...
    switch (configType) {
      ...
      case 'object':
        if (Array.isArray(config)) {
          ...
          return {
            forbidden: sanitizeConfigArray(config.forbidden || []),
          };
        }
        break;
      case 'undefined':
  ...
  visitor() {
    return {
      ElementNode(node) {
        let isForbiddenElement =
          FORBIDDEN_ELEMENTS.includes(node.tag) || this.config.forbidden.includes(node.tag);
