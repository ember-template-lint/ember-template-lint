...
const OPTIONAL_CONFIG_KEYS = new Set(['extendForbidden', 'message']);
...
function isValidConfigObjectFormat(config) {
  for (let key in config) {
    ...
    if ((key === 'forbidden' || key === 'extendForbidden') && !valueIsArray) {
      return false;
...
function makeForbiddenElements(config) {
  ...
  } else if (Object.prototype.hasOwnProperty.call(config, 'extendForbidden')) {
    return sanitizeConfigArray([...config.extendForbidden, ...FORBIDDEN_ELEMENTS]);
  }
