...
function createInvalidRoleErrorMessage(element) {
...
const ELEMENTS_DISALLOWING_PRESENTATION_OR_NONE_ROLE = [
  'a',
  'abbr',
  'applet',
  'area',
...
<<<<<<< HEAD
const DEFAULT_CONFIG = { catchInvalidRoles: false };

=======
>>>>>>> chore: update no invalid role to check valid role values
module.exports = class NoInvalidRole extends Rule {
  parseConfig(config) {
    ...
    switch (configType) {
      ...
      case 'object':
        return { catchInvalidRoles: config.catchInvalidRoles };
      case 'undefined':
        return false;
    }
    ...
    let errorMessage = createErrorMessage(
      ...
      [
        ...
        '    * `catchInvalidRoles` -- Whether invalid role values should be allowed (defaults to `true`)',
      ],
      config
    );
    ...
    return {
      ElementNode(node) {
        ...
        if (
          ...
          ELEMENTS_DISALLOWING_PRESENTATION_OR_NONE_ROLE.includes(node.tag)
        ) {
          if (ELEMENTS_DISALLOWING_PRESENTATION_OR_NONE_ROLE.includes(node.tag)) {
            this.log({
        ...
        if (!this.config.catchInvalidRoles) {
          return;
        }

        if (!VALID_ROLES.includes(roleValue.toLowerCase())) {
          this.log({
            message: createInvalidRoleErrorMessage(node.tag),
            line: node.loc && node.loc.start.line,
            column: node.loc && node.loc.start.column,
            source: this.sourceForNode(node),
          });
...
module.exports.createInvalidRoleErrorMessage = createInvalidRoleErrorMessage;
