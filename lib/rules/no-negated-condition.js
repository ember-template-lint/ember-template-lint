...
const ERROR_MESSAGE_DOUBLE_NEGATION = 'Remove unnecessary double negation.';

export default class NoNegatedCondition extends Rule {
  visitor() {
    function hasNestedNegation(node) {
      return node.params[0].params[0].path && node.params[0].params[0].path.original === 'not';
    }
    ...
    function fixNestedNotHelper(node) {
    ...
    function checkNode(node) {
      ...
      if (node.type === 'BlockStatement') {
        if (this.sourceForNode(node).startsWith('{{else ')) {
          ...
          // unless it has double negation.
          if (!hasNotHelper(node)) {
            return;
          }

          if (!hasNestedNegation(node)) {
            return;
          }
          if (this.mode === 'fix') {
            node.params[0] = node.params[0].params[0]; // Unwrap condition to remove `not`
            fixNestedNotHelper(node);
          } else {
            this.log({
              message: ERROR_MESSAGE_DOUBLE_NEGATION,
              node,
              isFixable: true,
            });
          }
      ...
      const hasDoubleNegation = hasNestedNegation(node);
      if (isIf && node.params[0].params[0].type === 'SubExpression' && !hasDoubleNegation) {
        ...
        // `not` helper is exception as we can easy simplify `(not (not...))`.
        return;
      }
      ...
      const message = hasDoubleNegation
        ? ERROR_MESSAGE_DOUBLE_NEGATION
        : isUnless
      ...
      if (this.mode === 'fix') {
        ...
        if (hasDoubleNegation) {
          fixNestedNotHelper(node);
          return; // removing double negation doesn't require any other changes
        }
