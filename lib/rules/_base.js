...
export default class {
  constructor(options) {
    ...
    this._disabledRules = new Set(); // Track disabled rules
    this._usedDisabledRules = new Set(); // Track which disabled rules were actually needed
  }
      ...
      if (instructionArgs) {
        ...
        if (m[1] === 'disable') {
          for (const name of names) {
            if (this._refersToCurrentRule(name)) {
              this._disabledRules.add(node);
            }
          }
        }
      }
  ...
  log(result) {
    // When a rule logs a violation, mark any disabled rule directives as used
    if (this._disabledRules.size > 0 && !this.isDisabled()) {
      for (let node of this._disabledRules) {
        this._usedDisabledRules.add(node);
      }
    }
