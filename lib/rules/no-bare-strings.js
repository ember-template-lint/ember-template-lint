...
const DEFAULT_ELEMENT_ATTRIBUTES = {
  ContactSupportLink: ['@emailSubject', '@intercomMessage', '@linkText'],
  Input: ['placeholder', '@placeholder'],
  TaskRetrySection: ['@description'],
  Textarea: ['placeholder', '@placeholder'],
  TooltipDecorator: ['@tooltipText'],
  img: ['alt'],
  input: ['placeholder'],
};
...
export default class NoBareStrings extends Rule {
  ...
  _checkNodeAndLog(node, additionalDescription, loc) {
    ...
    const bareStringsFile = fs.readFileSync('./bare-strings.json', 'utf8');
    let bareStringsObject = JSON.parse(bareStringsFile);
    ...
    switch (node.type) {
      case 'TextNode': {
        ...
        if (bareStringText) {
          const simplifiedPath = this.filePath
            .replace('/addon/', '/')
            .replace('/app/', '/')
            .replace('packages/', '')
            .replace('francium-', '')
          ...
          const bareStringTextFormatted = bareStringText
            .replace(/\s{2,}/g, ' ')
            .replace('\n', ' ')
            .trim();

          const key = `${simplifiedPath}__${_kebabCase(bareStringTextFormatted.slice(0, 50))}`;

          bareStringsObject = {
            ...bareStringsObject,
            [key]: bareStringTextFormatted,
          };
    ...
    fs.writeFileSync('bare-strings.yaml', stringify(bareStringsObject));
    fs.writeFileSync('bare-strings.json', JSON.stringify(bareStringsObject, null, 2));
  }
}
