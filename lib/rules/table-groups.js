...
function isValidComponentName(str) {
...
function getEffectiveChildren(node) {
  ...
  return [].concat(...unflattenedEffectiveChildren);
}
...
function isAllowedTableChild(node, config) {
  switch (node.type) {
    ...
    case 'ElementNode': {
      ...
      const possibleIndices = config.get(dasherize(node.tag)) || [];
      if (possibleIndices.length > 0) {
        return { allowed: true, indices: possibleIndices };
      }
  ...
  parseConfig(config) {
    ...
    switch (configType) {
      ...
      case 'object': {
        ...
        allowedComponentKeysByIndex.forEach((key, index) => {
          if (key in config) {
            const allowedComponents = config[key];
            if (Array.isArray(allowedComponents) && allowedComponents.every(isValidComponentName)) {
              allowedComponents.forEach((allowedComponent) => {
          ...
          for (const child of children) {
            ...
            if (indices.length > 0) {
              const newAllowedMinimumIndices = new Set(
                ...
                [].concat(
                  ...[...currentAllowedMinimumIndices].map((currentIndex) => {
