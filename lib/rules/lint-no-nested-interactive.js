...
module.exports = class LogNestedInteractive extends Rule {
  ...
  visitor() {
    ...
    let visitor = {
      enter(node) {
        ...
        } else if (this.hasParentNode() && !this.hasDetailsElementCase()) {
          this.log({
  ...
  hasDetailsElementCase() {
    return this.hasDetailsParentNode() && this.hasSummaryAsFirstDetailsChildrenNode();
  }

  hasSummaryAsFirstDetailsChildrenNode() {
    const elements = this._parentInteractiveNode.children.filter(
      element => element.type === 'ElementNode'
    );
    return elements.length && elements[0].tag === 'summary';
  }
