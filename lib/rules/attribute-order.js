...
export default class AttributeOrder extends Rule {
  ...
  makeTokenGroups(tokens) {
    ...
    for (const tokenType of Object.keys(tokens)) {
      tokenGroups[tokenType] = new TokenGroup(tokens[tokenType]);
    }
    return tokenGroups;
  }
  ...
  findUnalphabetizedToken(tokenGroup) {
    return tokenGroup.items[tokenGroup.unalphabetizedItemIndex(this)];
  }
  ...
  checkMustacheAttributesOrder(node) {
    ...
    if (this.config.alphabetize && tokenGroup.unalphabetizedItemIndex(this) >= 0) {
  ...
  checkElementAttributesOrder(node, { parentNode, parentKey }) {
    ...
    for (const tokenType of this.config.order) {
      ...
      if (
        ...
        tokenGroups[tokenType].unalphabetizedItemIndex(this) >= 0 &&
  ...
  orderAttribute({ tokenType, tokenGroups, node, parentNode, parentKey }) {
    ...
    if (this.mode === 'fix') {
      ...
      // Add comments into sorted array of tokens by inserting it
      // at the same position relative to the next token in original order.
      for (const comment of node[TokenType.COMMENTS]) {
        ...
        const toIndex = sorted.findIndex((node) => {
          return (
            this.calculateAttributeIndex(node) === this.getAllIndexes(tokenGroups)[fromIndex + 1]
          );
        });
