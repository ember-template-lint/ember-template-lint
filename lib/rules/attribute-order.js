...
export default class AttributeOrder extends Rule {
  ...
  addMetadata(tokens) {
    for (const attributeType of Object.keys(tokens)) {
      ...
      const indexes = tokens[attributeType].map(([attr]) => this.calculateAttributeIndex(attr));
      const alpha = tokens[attributeType].map(
        ([attr]) => this.sourceForNode(attr).match(/([^A-Za-z]*)([\w-]*)/)[2]
      );
      tokens[attributeType] = {
        items: tokens[attributeType],
        exists: tokens[attributeType].length !== 0,
  ...
  findCurrentElement(attributeType, nodeTokens) {
    return nodeTokens[attributeType].items.find(
      ...
      ([item]) => this.calculateAttributeIndex(item) === nodeTokens[attributeType].last_index
    );
  }
  ...
  checkMustacheAttributesOrder(node) {
    ...
    if (this.config.alphabetize && nodeTokens[attributeType].unalphabetized_item_pos >= 0) {
      const [item] = this.findUnalphabetizedElement(attributeType, nodeTokens);
  ...
  orderAttribute({ attributeType, nodeTokens, node, parentNode, parentKey }) {
    const [item] = this.findCurrentElement(attributeType, nodeTokens);
    ...
    if (this.mode === 'fix') {
      ...
      for (const comment of node[AttributeType.COMMENTS]) {
        ...
        const fromIndex = nodeTokens.indexes.indexOf(this.calculateAttributeIndex(token));
