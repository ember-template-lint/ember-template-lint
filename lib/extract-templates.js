...
export function replaceTemplates(originalSource, transforms) {
  ...
  // templateInfo is https://github.com/embroider-build/content-tag/blob/main/index.d.ts#L7
  // "Parsed"
  for (let { templateInfo, transformed } of transforms) {
     ...
     */
    let originalContent = buffer.slice(templateInfo.range.start, templateInfo.range.end).toString();
    let originalLength = originalContent.length;
    let originalBeforeContent = buffer.slice(0, templateInfo.range.start).toString();
     ...
     * exploration.
    */
    let openingTag = buffer.slice(templateInfo.startRange.start, templateInfo.startRange.end).toString();
    let closingTag = buffer.slice(templateInfo.endRange.start, templateInfo.endRange.end).toString();
 ...
 * @returns {TemplateInfo}
 */
function makeTemplateInfo(location, template, templateInfo) {
...
export function coordinatesOf(text, templateInfo) {
   ...
   */
  let { range: byteRange } = templateInfo;
