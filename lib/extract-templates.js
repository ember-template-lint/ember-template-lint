...
export function extractTemplates(moduleSource, relativePath) {
  ...
  if (!isGlimmer) {
    ...
    if (isHbs) {
      return [
        {
          ...makeTemplateInfo(LOCATION_START, moduleSource),
  ...
  try {
    parsed= p.parse(moduleSource, { inline_source_map: false, filename: relativePath });
  } catch (error) {
    console.log(error);
    throw error;
  }
  ...
  let result = parsed.map((templateInfo) => {
    return makeTemplateInfo(
      coordinatesOf(moduleSource, templateInfo),
...
export function replaceTemplates(originalSource, transforms) {
  ...
  for (let { templateInfo, transformed } of transforms) {
    ...
    let originalContent = buffer.slice(byteRange.start, byteRange.end).toString();
    let originalLength = originalContent.length;
    let originalBeforeContent = buffer.slice(0, byteRange.start).toString();
 ...
 * @returns {TemplateInfo}
 */
function makeTemplateInfo({ line, column, start, end, columnOffset }, template, templateInfo) {
  return {
    line,
    start,
    end,
