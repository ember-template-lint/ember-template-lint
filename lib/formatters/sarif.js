...
class SarifPrinter {
  ...
  print(results) {
    ...
    let nextRuleIndex = 0;
    const sarifResults = [];
    const embedFileContents = process.env.SARIF_EMBER_TEMPLATE_LINT_EMBED === 'true';
            ...
            if (message.rule && message.rule !== 'global') {
              ...
              if (typeof sarifRules[message.rule] === 'undefined') {
                ...
                sarifRules[message.rule] = {
                  id: message.rule,
                  helpUri: `https://github.com/ember-template-lint/ember-template-lint/blob/master/docs/rule/${message.rule}.md`,
                };
              }
