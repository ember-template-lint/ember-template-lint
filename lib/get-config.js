...
const SEVERITY_LEVEL_IGNORE = 'off';
const SEVERITY_LEVEL_WARNING = 'warn';
...
function processRules(config) {
  for (let key in config.rules) {
    ...
    // array of severity and custom rule config
      ...
      ruleConfig.severity = severityConfig.severity;
      ruleConfig.config = severity === SEVERITY_LEVEL_IGNORE ? false : ruleData[1];
      config.rules[key] = ruleConfig;
    }
  }
}
...
module.exports = { getProjectConfig, getConfigForFile, ERROR_SEVERITY, WARNING_SEVERITY };
