...
const RULE_NAME_DEPRECATIONS = [
  {
    oldName: 'inline-styles',
    newName: 'no-inline-styles'
  },
  {
    oldName: 'bare-strings',
    newName: 'no-bare-strings'
  },
  {
    oldName: 'html-comments',
    newName: 'no-html-comments'
  },
  {
    oldName: 'invalid-interactive',
    newName: 'no-invalid-interactive'
  },
  {
    oldName: 'nested-interactive',
    newName: 'no-nested-interactive'
  },
  {
    oldName: 'triple-curlies',
    newName: 'no-triple-curlies'
  },
  {
    oldName: 'unused-block-params',
    newName: 'no-unused-block-params'
  }
];
...
function validateRules(config, options) {
  ...
  for (let key in config.rules) {
    if (!config.loadedRules[key]) {
      if (RULE_NAME_DEPRECATIONS.map(a => a.oldName).indexOf(key) > -1) {
        deprecatedNamesFound.push(RULE_NAME_DEPRECATIONS.find(a => a.oldName === key));
      } else {
        invalidKeysFound.push(key);
      }
    }
  ...
  if (deprecatedNamesFound.length > 0) {
    logger.log(chalk.yellow(`Deprecated rule names found: ${JSON.stringify(deprecatedNamesFound, null, 4)}`));
  }
}
