...
async function requirePlugin(workingDir, pluginName, fromConfigPath) {
  ...
  try {
    ...
    const packageJSON = JSON.parse(fs.readFileSync(item, 'utf8'));
  ...
  try {
    ...
    const { default: plugin } = await import(pluginURL.toString());
    return plugin;
  } catch (error) {
...
export async function getProjectConfig(workingDir, options) {
  ...
  if (source._processed) {
    ...
    // invoke `setup` on rules (if exists)
    const ruleSetupPromises = [];
    Object.entries(config.loadedRules).forEach(([ruleName, ruleKlass]) => {
      if (
        ruleKlass &&
        (typeof ruleKlass === 'function' || typeof ruleKlass === 'object') &&
        'setup' in ruleKlass &&
        typeof ruleKlass['setup'] === 'function'
      ) {
        ruleSetupPromises.push(ruleKlass['setup'](config.rules[ruleName]));
      }
    });

    await Promise.all(ruleSetupPromises);
