...
/**
 * Returns the rules config in conjunction with overrides configuration.
 * @param {*} config
 * @param {*} filePath
 */
function getRulesConfig(config, filePath) {
  let configuredRules = config.rules;
  let overrides = config.overrides;
  if (filePath && overrides) {
    let overridesObj =
      overrides.find(override => override.files && micromatch.isMatch(filePath, override.files)) ||
      {};
    // If there is an override present, then the overridden ruleset takes precedence.
    return Object.assign({}, configuredRules, overridesObj.rules);
  }
  ...
  if (source._processed) {
    ...
    if (config.overrides) {
      for (const override of config.overrides) {
        ...
        const containsValidKeys = overrideKeys.every(item =>
          SUPPORTED_OVERRIDE_KEYS.includes(item)
        );
        if (!containsValidKeys) {
          throw new Error(
            'Override object cannot contain anything other than the `files` and `rules` property.'
          );
        }
...
module.exports = { getConfig, getRulesConfig };
